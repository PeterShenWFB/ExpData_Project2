...
bcTotals <- aggregate(Emissions ~ year, baltimore, sum)
# ensure ggplot2 library is loaded
library(ggplot2)

# ggplot
gbc <- ggplot(bcTotals, aes(year, Emissions))
gbc <- gbc + geom_line() +
        xlab("year") +
        ylab(expression('Total PM'[2.5]*" Emissions")) +
        ggtitle('Total Emissions from motor vihecle (type is ON-ROAD) in Baltimore City, Maryland from 1999 to 2008')
print(gbc)
